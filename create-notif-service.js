...
export class NotificationService implements INotificationService {
  ...
  getProvider(name: string): NotificationProvider | undefined {
    // We expect providers to be injected; if one is missing, return undefined so it can be handled
    return this.providers.get(name.toLowerCase()) || this.providers.get("console");
  }
    ...
    for (const action of claimedActions) {
      ...
      if (!provider) {
         result = {
           ...
           error: \`No provider registered for \${action.provider}\`,
         };
      } else {
  ...
  private async handleFollowupStateConfirmation(action: NotificationActionRow, confirmedBy: string): Promise<void> {
...
fs.writeFileSync('src/services/impl/NotificationService.ts', serviceCode);
try {
  fs.unlinkSync('src/services/impl/NoOpNotificationService.ts');
} catch(e) {}
