...
export async function POST(request: NextRequest, { params }: { params: Promise<{ decisionId: string }> }) {
  ...
  const result = await ServiceFactory.getDecisionService(client).transition({
    decisionId, targetStatus: "READY_FOR_REVIEW", actor: resolveActor(scoped.identity, body.actor), idempotencyKey: typeof body.idempotencyKey === "string" ? body.idempotencyKey : "", metadata: body.metadata as Record<string, unknown> | undefined,
  });
