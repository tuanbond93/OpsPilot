...
export async function POST(request: NextRequest, { params }: { params: Promise<{ decisionId: string }> }) {
  ...
  const result = await ServiceFactory.getDecisionService(client).recordOutcome({
    decisionId, status: body.status as never, observedOutcome: typeof body.observedOutcome === "string" ? body.observedOutcome.slice(0, 5000) : "", measuredAt: typeof body.measuredAt === "string" ? body.measuredAt : "",
    evidenceRefs: Array.isArray(body.evidenceRefs) ? body.evidenceRefs.filter((item): item is string => typeof item === "string").slice(0, 50) : [], inconclusiveReason: typeof body.inconclusiveReason === "string" ? body.inconclusiveReason.slice(0, 5000) : undefined,
