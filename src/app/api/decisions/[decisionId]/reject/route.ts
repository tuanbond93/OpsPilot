...
export async function POST(request: NextRequest, { params }: { params: Promise<{ decisionId: string }> }) {
  ...
  const result = await ServiceFactory.getDecisionService(client).transition({
    decisionId, targetStatus: "REJECTED", actor: resolveActor(scoped.identity, body.actor), idempotencyKey: typeof body.idempotencyKey === "string" ? body.idempotencyKey : "",
    rejectReason: typeof body.rejectReason === "string" ? body.rejectReason.slice(0, 5000) : undefined, metadata: body.metadata as Record<string, unknown> | undefined,
  });
