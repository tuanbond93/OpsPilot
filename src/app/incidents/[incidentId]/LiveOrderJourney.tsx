...
export function LiveOrderJourney({ orderCode, onResolved }: { orderCode: string; onResolved?: (data: ApiData) => LiveOrderTracking | void }) {
  ...
  return <div aria-busy={loading} className="space-y-5">
    ...
    {(data.phase === "DELIVERING" || data.phase === "COMPLETED" || data.deliveryStartedAt || data.endDeliveryAt || data.endSuccessAt) && <dl className="grid gap-3 rounded-xl border border-emerald-500/20 bg-emerald-950/10 p-4 text-sm sm:grid-cols-2"><div><dt className="text-slate-400">Log xuất giao/bắt đầu giao</dt><dd className="mt-1 font-mono font-semibold text-cyan-200">{formatTime(data.deliveryStartedAt)}</dd>{data.deliveryStartedAtInferred && <p className="mt-1 text-xs text-amber-200">Nguồn không có log gán shipper riêng; mốc này được suy ra từ sự kiện giao đầu tiên.</p>}{!data.deliveryStartedAt && <p className="mt-1 text-xs text-amber-200">Nguồn GHN chưa trả log xuất giao riêng.</p>}</div><div><dt className="text-slate-400">Giao thành công</dt><dd className="mt-1 font-mono font-semibold text-emerald-200">{formatTime(data.endSuccessAt || data.endDeliveryAt)}</dd>{!data.endSuccessAt && !data.endDeliveryAt && <p className="mt-1 text-xs text-amber-200">Nguồn GHN chưa trả log giao thành công.</p>}</div></dl>}
