...
import { AlertTriangle, BrainCircuit, Check, Circle, LoaderCircle, RefreshCw, Truck } from "lucide-react";
...
export function LiveOrderJourney({ orderCode, onResolved }: { orderCode: string; onResolved?: (data: ApiData) => LiveOrderTracking | void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        void fetch(`/api/incidents/${encodeURIComponent(incidentId)}/orders/${encodeURIComponent(orderCode)}/live-status`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      } catch (bridgeError) {
  ...
  return <div aria-busy={loading} className="space-y-5">
    ...
    {data.deliverWarehouseId && <dl className="grid gap-3 rounded-xl border border-indigo-500/30 bg-indigo-950/20 p-4 text-sm sm:grid-cols-2"><div><dt className="text-slate-400">Kho đang giữ đơn</dt><dd className="mt-1 font-semibold text-cyan-100">{data.currentWarehouseName || `Kho ${data.currentWarehouseId || "chưa xác định"}`}</dd></div><div><dt className="text-slate-400">Kho giao cuối theo log GHN</dt><dd className="mt-1 font-semibold text-indigo-100">{data.deliverWarehouseName || `Kho ${data.deliverWarehouseId}`}</dd><p className="mt-1 text-xs text-slate-400">Mã kho: {data.deliverWarehouseId}{data.currentWarehouseId && data.currentWarehouseId !== data.deliverWarehouseId ? " · chưa có log xác nhận đã xuất từ kho đang giữ" : ""}</p></div></dl>}
    {(data.deliveryStartedAt || data.endDeliveryAt || data.endSuccessAt) && <dl className="grid gap-3 rounded-xl border border-emerald-500/20 bg-emerald-950/10 p-4 text-sm sm:grid-cols-2"><div><dt className="text-slate-400">Log gán/bắt đầu giao</dt><dd className="mt-1 font-mono font-semibold text-cyan-200">{formatTime(data.deliveryStartedAt)}</dd>{data.deliveryStartedAtInferred && <p className="mt-1 text-xs text-amber-200">Nguồn thiếu log gán shipper riêng; mốc này được suy ra từ sự kiện giao mới nhất.</p>}</div><div><dt className="text-slate-400">Giao thành công</dt><dd className="mt-1 font-mono font-semibold text-emerald-200">{formatTime(data.endSuccessAt || data.endDeliveryAt)}</dd></div></dl>}
