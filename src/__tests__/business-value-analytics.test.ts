...
import { buildBusinessValueReport, type BusinessValueCaseEvidence } from "@/domain/business-value/analytics";
...
const action = (id: string, type: "FIRST_PUSH" | "SECOND_PUSH" | "ESCALATION", processedAt: string, outcome: "DELIVERED" | "FAILED" = "DELIVERED") => ({
  id, action_type: type, provider: "telegram", target_type: "WAREHOUSE" as const, payload: {}, status: "SENT" as const,
  priority: "high" as const, retry_count: 0, max_retry: 1, scheduled_at: processedAt, processed_at: processedAt, outcome,
});

describe("business value analytics read model", () => {
  it("attributes a resolution after a confirmed first push only", () => {
    const report = buildBusinessValueReport([{ followupCase: { ...base("a", "RESOLVED"), resolved_at: "2026-09-12T04:00:00.000Z" }, actions: [action("a1", "FIRST_PUSH", "2026-09-12T02:00:00.000Z")] }]);
  ...
  it("keeps an unresolved first-push case out of resolution outcomes", () => {
    const report = buildBusinessValueReport([{ followupCase: base("b"), actions: [action("b1", "FIRST_PUSH", "2026-09-12T02:00:00.000Z")] }]);
  ...
  it("attributes second-push and escalation resolutions as mutually exclusive", () => {
    const report = buildBusinessValueReport([
      { followupCase: { ...base("c", "RESOLVED"), resolved_at: "2026-09-12T05:00:00.000Z" }, actions: [action("c1", "FIRST_PUSH", "2026-09-12T02:00:00.000Z"), action("c2", "SECOND_PUSH", "2026-09-12T03:00:00.000Z")] },
      { followupCase: { ...base("d", "RESOLVED"), resolved_at: "2026-09-12T06:00:00.000Z" }, actions: [action("d1", "FIRST_PUSH", "2026-09-12T02:00:00.000Z"), action("d2", "SECOND_PUSH", "2026-09-12T03:00:00.000Z"), action("d3", "ESCALATION", "2026-09-12T04:00:00.000Z")] },
    ]);
  ...
  it("excludes suppressed cases and records deduplication events", () => {
    const report = buildBusinessValueReport([{ followupCase: base("e"), suppressionReason: "CUSTOMER_APPOINTMENT", actionEvents: [{ id: "ev", action_id: "x", event_type: "ACTION_DEDUPLICATED", attempt_number: 0 }] }]);
  ...
  it("never treats a pending or failed dispatch as confirmed", () => {
    ...
    const report = buildBusinessValueReport([{ followupCase: { ...base("f", "RESOLVED"), resolved_at: "2026-09-12T04:00:00.000Z" }, actions: [pending] }]);
  ...
  it("identifies invalid ordering and preserves missing timestamp measurements as N/A", () => {
    const report = buildBusinessValueReport([{ followupCase: base("g"), actions: [action("g2", "SECOND_PUSH", "2026-09-12T02:00:00.000Z")] }]);
  ...
  it("accepts caller-provided scope selections without cross-scope aggregation", () => {
    const all: BusinessValueCaseEvidence[] = [{ followupCase: base("mb3"), scope: { warehouseId: "MB3" } }, { followupCase: base("hcm"), scope: { warehouseId: "HCM" } }];
