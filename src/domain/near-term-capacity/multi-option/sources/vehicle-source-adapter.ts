...
export type RateBasis = "TRIP" | "HOUR" | "DAY" | "KG";
export type VehicleRateEvidenceStatus = "MEASURED" | "GOVERNED_RATE" | "MODELED" | "UNKNOWN";
...
export type VehicleCapacityEvidenceStatus = "MEASURED" | "GOVERNED" | "MODELED" | "UNKNOWN";
...
export type VehicleAvailabilityEvidenceStatus = "MEASURED" | "GOVERNED" | "MODELED" | "UNKNOWN";
...
export interface VehicleSourceAdapter {
  getVehicleRate(warehouseId: string, vehicleClass?: string): Promise<VehicleRateEvidence> | VehicleRateEvidence;
...
export class GovernedVehicleSourceAdapter implements VehicleSourceAdapter {
  ...
  getVehicleRate(warehouseId: string, vehicleClass = "STANDARD_EXTERNAL_TRUCK"): any {
    if (this.config.db) {
      return this.queryDbRate(warehouseId, vehicleClass);
    }
    return this.getInMemoryRate(warehouseId, vehicleClass);
  }
  ...
  getVehicleEvidence(warehouseId: string, vehicleClass?: string): any {
    ...
    const rate = this.getInMemoryRate(warehouseId, targetClass);
  ...
  private async queryDbRate(warehouseId: string, vehicleClass: string): Promise<VehicleRateEvidence> {
    try {
      ...
      const matched = data.find((r: any) => r.warehouse_id === warehouseId) || data[0];
      const now = Date.now();
  ...
  private getInMemoryRate(warehouseId: string, vehicleClass: string): VehicleRateEvidence {
    ...
    const effectiveTime = matched.effective_at ? new Date(matched.effective_at).getTime() : 0;
    const expiresTime = matched.expires_at ? new Date(matched.expires_at).getTime() : Infinity;

    const isPastExpiry = Boolean(matched.expires_at && now > expiresTime);
    const isPastMaxAge = effectiveTime > 0 && now - effectiveTime > maxAgeMs;
    const isExplicitlyStale = matched.is_stale === true;

    if (isPastExpiry || isPastMaxAge || isExplicitlyStale) {
      return {
        vehicle_class: matched.vehicle_class,
        warehouse_or_scope: matched.warehouse_or_scope || warehouseId,
        route_or_area: matched.route_or_area || null,
        rate_vnd: null,
        rate_basis: matched.rate_basis,
        source_ref: matched.source_ref,
        ...
        stale_reason: isPastExpiry
          ? `Biểu phí đã hết hạn vào ngày ${matched.expires_at}`
  ...
  private async queryDbEvidence(warehouseId: string, vehicleClass?: string): Promise<VehicleEconomicsAndCapacityResult> {
    ...
    const targetClass = vehicleClass || availability.vehicle_class || "STANDARD_EXTERNAL_TRUCK";
    const [rate, capacity] = await Promise.all([
      this.queryDbRate(warehouseId, targetClass),
